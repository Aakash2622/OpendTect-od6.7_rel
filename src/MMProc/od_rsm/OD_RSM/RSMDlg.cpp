...
void CRSMDlg::OnMenuStart()
{
    ...
    {
	CString errmsg( "Start service failed: Could not locate file at\n" );
	errmsg += GetServiceFile();
	mErrMsg( errmsg )
    }
...
void CRSMDlg::LogMessage( CString msg )
{
    ...
    CString timemsg;
    timemsg.Format(L"[%0.2d/%0.2d/%0.2d %0.2d:%0.2d:%0.2d] ",dd,mm,yy,h,m,s);
    timemsg += msg;	
    loglist_.AddString( timemsg );
...
const char* GetServiceFile()
{
    ...
    const char* bin32 = "bin\\win32\\Release";
    const char* bin64 = "bin\\win64\\Release";
